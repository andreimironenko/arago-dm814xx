...
if [ ! -f $sdkdir/filesystem/dvsdk-am180x-evm-rootfs.tar.gz ]; then
  echo "ERROR: failed to find rootfs $sdkdir/filesystem/dvsdk-am180x-evm-rootfs.tar.gz"
  exit 1;
fi
...
execute "tar zxf $sdkdir/filesystem/dvsdk-am180x-evm-rootfs.tar.gz -C /tmp/sdk/$$"
sync
