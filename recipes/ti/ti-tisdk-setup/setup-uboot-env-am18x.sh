...
baseargs="console=ttyS2,115200n8 noinitrd rw mpurate=1000"
extendbaseargs=""
...
#    echo "send \"$videoargs3 \c\"" >> $minicomfilepath
    if [ "$fs" -eq "1" ]; then
        echo "send \"$fsnfsargs1\c\"" >> $minicomfilepath
        echo "send \"$fsnfsargs2\c\"" >> $minicomfilepath
        echo "send \"$fsnfsargs3\c\"" >> $minicomfilepath
        echo "send \"$fsnfsargs4 \c\"" >> $minicomfilepath
